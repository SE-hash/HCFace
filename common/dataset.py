...
class TestImageDataset(BaseImageDataset):
    def __init__(self, dataset_name, transforms=None):
        super(TestImageDataset, self).__init__(dataset_name, transforms=transforms)
        self.ids = self.data[:, 0].astype(int)
        self.ages = self.data[:, 2].astype(np.float32)
...
class MORPHDataset(tordata.Dataset):
    def __init__(self, dataset_name, transforms=None):
        self.transforms = transforms
        self.root = 'your/path'
        df = pd.read_csv(self.root)
        self.data = df.values
        self.image_list = np.array([osp.join('your/path', x) for x in self.data[:, 2]])

    def __len__(self):
        return len(self.image_list)


class MORPHTrainDataset(MORPHDataset):
    def __init__(self, transforms=None):
        ...
        self.image_list = np.array(
            [osp.join('your/path', x) for x in self.data[:, 2]])
        self.ids = [int(id.split('_')[0]) for id in self.data[:,2].astype(str)]
        self.classes = np.unique(self.ids)
        self.ages = self.data[:,0].astype(np.float32)
        self.genders = self.data[:, 1].astype(int)
